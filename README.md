# Hermès 澳洲 Lindy 库存监控

每 10 分钟自动检查爱马仕澳洲官网 Lindy 是否上货（比如 Lindy II mini），
一上线就**发邮件通知你**并附商品链接。跑在 GitHub Actions 上，**免费、不用开电脑**。
...
| `MAIL_TO`            | 收通知的邮箱 | ✅ |
| `LISTING_URL`        | 留空用默认搜索页 | 可选 |
| `SLUG_FILTER`        | 留空默认 `lindy` | 可选 |
| `MATCH_KEYWORD`      | 留空默认 `mini`（邮件标题高亮用） | 可选 |
